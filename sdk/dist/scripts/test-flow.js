...
const attestors = Array.from({ length: 5 }, () => web3_js_1.Keypair.generate());
const resolver = web3_js_1.Keypair.generate(); // would be multisig in prod
// ── Helpers ───────────────────────────────────────────────────────────────────
...
function sdkFor(keypair) {
    return (0, index_1.createSDKFromKeypair)(keypair, RPC_URL, {
        escrow: escrowIdl,
...
async function main() {
    ...
    const AMOUNT = BigInt(web3_js_1.LAMPORTS_PER_SOL); // 1 SOL
    const THRESHOLD = 3;
    ...
    const { signature: createSig, escrowPubkey, registryPubkey } = await payerSDK.setupEscrow({
    ...
    for (let i = 0; i < THRESHOLD; i++) {
        ...
        pass(`Attestor ${i + 1} submitted (${registry.attestationCount}/${THRESHOLD}) tx=${sig.slice(0, 16)}...`);
    }
    ...
    const statusMap = await payerSDK.attestation.getAttestationStatus(ESCROW_ID_1, attestors.map((a) => a.publicKey));
    let attestedCount = 0;
    statusMap.forEach((v) => { if (v)
        attestedCount++; });
    ...
    try {
        ...
        assert.ok(e.message.includes("DisputeWindowActive") || e.message.includes("custom program error"), `Expected DisputeWindowActive, got: ${e.message}`);
    ...
    const releaseSig = await receiverSDK.escrow.releaseFunds(ESCROW_ID_1, receiver.publicKey);
    ...
    try {
        ...
        assert.ok(e.message.includes("InvalidStatus") || e.message.includes("custom program error"), `Expected InvalidStatus, got: ${e.message}`);
    ...
    try {
        ...
        assert.ok(e.message.includes("NotAuthorizedAttestor") || e.message.includes("custom program error"), `Expected NotAuthorizedAttestor, got: ${e.message}`);
    ...
    try {
        ...
        assert.ok(e.message.includes("AlreadyAttested") || e.message.includes("custom program error"), `Expected AlreadyAttested, got: ${e.message}`);
    ...
    try {
        ...
        assert.ok(e.message.includes("OnlyPayerCanDispute") || e.message.includes("custom program error"), `Expected OnlyPayerCanDispute, got: ${e.message}`);
